...
const services = [
  { title: "Запуск и валидация идеи", description: "Проверка гипотез, customer development и уточнение продукта." },
  { title: "Разработка продукта и прототипирование", description: "MVP, прототипы и итеративная разработка продукта." },
  { title: "Выход на рынок и первые клиенты", description: "Go-to-market стратегия и привлечение первых клиентов." },
  { title: "Акселерация и менторская поддержка", description: "Участие в акселерационных программах и менторство." },
];

export default function StartupsAccelerationPage() {
  return (
      ...
      <section className="relative pt-24 pb-16 bg-gradient-to-b from-[#e8f5f3] to-white overflow-hidden">
        <div className="container-kept">
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8">
            ...
            <ChevronRight className="w-4 h-4" />
            <span className="text-[#02818a]">Акселерация и запуск</span>
          </nav>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#02818a]/10 text-[#02818a] text-sm font-medium mb-6">
                <Sparkles className="w-4 h-4" /> Направление
              </div>
              <h1 className="text-4xl lg:text-5xl font-bold text-[#02818a] mb-6">Акселерация и запуск</h1>
            ...
            <div className="hidden lg:flex justify-center">
              <div className="relative w-72 h-72">
                ...
                <div className="absolute inset-6 bg-[#02818a]/20 rounded-full flex items-center justify-center">
                  <Sparkles className="w-24 h-24 text-[#02818a]" />
                </div>
              </div>
            </div>
          </motion.div>
              ...
              <Link href="/services/startups/scaling" className="p-6 bg-white border border-gray-200 hover:bg-[#f1f2f4] transition-colors group">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Рост выручки и новые рынки</p>
                  </div>
                  <ArrowRight className="w-5 h-5 text-gray-300 group-hover:text-[#02818a] group-hover:translate-x-1 transition-all" />
                </div>
              </Link>
              <Link href="/services/startups/investment" className="p-6 bg-white border border-gray-200 hover:bg-[#f1f2f4] transition-colors group">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-2 group-hover:text-[#02818a] transition-colors">Привлечение инвестиций</h3>
                    <p className="text-sm text-gray-500">Pitch и работа с инвесторами</p>
                  </div>
                  <ArrowRight className="w-5 h-5 text-gray-300 group-hover:text-[#02818a] group-hover:translate-x-1 transition-all" />
                </div>
              </Link>
      ...
      <section className="py-16 lg:py-20 bg-[#02818a] relative overflow-hidden">
        <div className="container-kept relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div>
              <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">Запускаете стартап?</h2>
              <p className="text-white/70 max-w-xl">Поможем с валидацией, продуктом и выходом на рынок.</p>
            </div>
