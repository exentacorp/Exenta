...
import { Scale, ArrowRight, ChevronRight, CheckCircle } from "lucide-react";

const services = [
  { title: "Технологическая экспертиза", description: "Оценка проектных решений, технологий и соответствия техническим регламентам." },
  { title: "Правовая экспертиза", description: "Проверка соответствия проектной документации законодательству и разрешительным процедурам." },
  { title: "Соответствие нормативам", description: "Контроль соблюдения СНиП, ГОСТ, отраслевых стандартов и требований заказчика." },
  { title: "Сопровождение споров", description: "Подготовка заключений для судебных и досудебных споров по строительным вопросам." },
];

export default function EngineeringTechExpertisePage() {
  return (
      ...
      <section className="relative pt-24 pb-16 bg-gradient-to-b from-[#e8f5f3] to-white overflow-hidden">
        <div className="container-kept">
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8">
            ...
            <ChevronRight className="w-4 h-4" />
            <span className="text-[#02818a]">Технологическая и правовая экспертиза</span>
          </nav>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-4xl lg:text-5xl font-bold text-[#02818a] mb-6">
                Технологическая и правовая экспертиза
              </h1>
            ...
            <div className="hidden lg:flex justify-center">
              <div className="relative w-72 h-72">
                ...
                <div className="absolute inset-6 bg-[#02818a]/20 rounded-full flex items-center justify-center">
                  <Scale className="w-24 h-24 text-[#02818a]" />
                </div>
              </div>
            </div>
          </motion.div>
        ...
        <div className="container-kept">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <Link href="/services/engineering/construction-management" className="p-6 bg-white border border-gray-200 hover:bg-[#f1f2f4] transition-colors group">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-2 group-hover:text-[#02818a] transition-colors">Управление строительством</h3>
                    <p className="text-sm text-gray-500">Контроль сроков и подрядчиков</p>
                  </div>
                  <ArrowRight className="w-5 h-5 text-gray-300 group-hover:text-[#02818a] group-hover:translate-x-1 transition-all" />
                </div>
              </Link>
              <Link href="/services/engineering/pre-project" className="p-6 bg-white border border-gray-200 hover:bg-[#f1f2f4] transition-colors group">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-2 group-hover:text-[#02818a] transition-colors">Предпроектный анализ и ТЭО</h3>
                    <p className="text-sm text-gray-500">Обоснование до проектирования</p>
                  </div>
                  <ArrowRight className="w-5 h-5 text-gray-300 group-hover:text-[#02818a] group-hover:translate-x-1 transition-all" />
                </div>
              </Link>
      ...
      <section className="py-16 lg:py-20 bg-[#02818a] relative overflow-hidden">
        <div className="container-kept relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div>
              <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">Нужна экспертиза проекта?</h2>
              <p className="text-white/70 max-w-xl">Проведём техническую и правовую экспертизу.</p>
            </div>
