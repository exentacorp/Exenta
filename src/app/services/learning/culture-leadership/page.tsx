...
import { Award, ArrowRight, ChevronRight, CheckCircle } from "lucide-react";

const services = [
  { title: "Трансформация корпоративной культуры", description: "Изменение культуры через обучение, ценности и практики." },
  { title: "Развитие лидерства", description: "Программы развития лидерских компетенций и управленцев." },
  { title: "Управление изменениями", description: "Сопровождение изменений через обучение и коммуникации." },
  { title: "Командообразование", description: "Тимбилдинг и развитие командной эффективности." },
];

export default function LearningCultureLeadershipPage() {
  return (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#02818a]/10 text-[#02818a] text-sm font-medium mb-6">
                <Award className="w-4 h-4" /> Направление
              </div>
              <h1 className="text-4xl lg:text-5xl font-bold text-[#02818a] mb-6">Культура и развитие лидерства</h1>
              <p className="text-lg text-gray-600 leading-relaxed">
                Трансформация корпоративной культуры, развитие лидерства и управление изменениями через обучение.
              </p>
            </div>
            <div className="hidden lg:flex justify-center">
              <div className="relative w-72 h-72">
                ...
                <div className="absolute inset-6 bg-[#02818a]/20 rounded-full flex items-center justify-center">
                  <Award className="w-24 h-24 text-[#02818a]" />
                </div>
              </div>
            </div>
          </motion.div>
              ...
              <Link href="/services/learning/programs" className="p-6 bg-white border border-gray-200 hover:bg-[#f1f2f4] transition-colors group">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Программы и учебные модули</p>
                  </div>
                  <ArrowRight className="w-5 h-5 text-gray-300 group-hover:text-[#02818a] group-hover:translate-x-1 transition-all" />
                </div>
              </Link>
      ...
      <section className="py-16 lg:py-20 bg-[#02818a] relative overflow-hidden">
        <div className="container-kept relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div>
              <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">Развитие лидерства или трансформация культуры?</h2>
              <p className="text-white/70 max-w-xl">Поможем с программами развития и изменением культуры.</p>
            </div>
